...
const SubCourseButtonsBlock = inject('userStore', 'aprogressStore', 'modalStore', 'asubCourseStore')(observer((store) => {
        const {aprogressStore, modalStore, asubCourseStore} = store
        ...
        const getNotActive = () => {
            return <div className={'SubCourses'}>
                <div className={'SubCourses__Title'}>
                    <span>подкурсы </span>
                    {aprogressStore.courseData?.draft &&
        ...
        const getActive = () => {
            return <>{accordionStatus ? <div className={'SubCourses'}>
                ...
                <button type="button"
                        className={`btn btn-outline-dark SubCourses__ButtonSubActive`}
                        onClick={modalStore.ASubCourseEditModalShow}>
                    редактировать
                </button>
                <button type="button"
                        className={`btn btn-outline-dark SubCourses__ButtonSubActive`}
